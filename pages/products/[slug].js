...
const productPage = ({ product, roundProducts, error }) => {
      ...
      <section className="food_section my-5">
        <div className="container">
          <div className="row gx-3">
            {roundProducts &&
              roundProducts.map((item, index) => (
                <div key={index} className="col-sm-6 col-lg-3">
                  <Product prodctInfo={item} />
                </div>
              ))}
          </div>
        </div>
...
export async function getServerSideProps({ query }) {
  try {
    ...
    const resRound = await axios.get("random-products?count=4");
    return {
      props: {
        product: res.data.data,
        roundProducts: resRound.data.data,
      },
    };
  } catch (err) {
    return {
